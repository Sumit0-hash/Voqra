...
export function AppProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        if (isSignedIn && clerkUser) {
            const mappedUser: User = {
                ...
                bio: (clerkUser.publicMetadata?.bio as string) || "Hey there! I am using Voqra.",
                isOnline: true,
    ...
    const fetchStories = useCallback(async ()=>{
        try {
            ...
            // Handle the fetched stories data
        } catch (error) {
                ...
                ws.onmessage =(e)=>{
                    ...
                    if(event.type === "message"){
                        ...
                        setConversations((prev)=>{
                            ...
                            return prev.map((c)=> c._id === incoming.conversationId ? {...c, lastMessage: incoming, updatedAt: incoming.createdAt} : c).sort((a,b)=> new Date(b.updatedAt).getTime() - new Date(b.updatedAt).getTime())
                    ...
                    if(event.type === "user_update"){
                        ...
                        if(updated){
                            ...
                            setConversations((prev)=> prev.map((c)=>(c.participant?._id === updated._id ? {...c, participant: updated}: c))
                            );
    ...
    return (
        <AppContext.Provider value={{ auth, logout, updateUser, users, setUsers, conversations, setConversations, selectedConversation, setSelectedConversation, messages, setMessages, userStories, setUserStories, fetchStories, typingUsers, sendWsEvent }}>
            {children}
        </AppContext.Provider>
    )
}
